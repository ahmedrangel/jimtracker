...
<template>
  <div :style="style ? style : defaultStyle" class="text-center text-xs md:text-base absolute border border-blue-500/80 rounded-lg w-max overflow-hidden px-0 pointer-events-none z-100 transition-all duration-300 ease-in-out">
    <template v-if="content">
      <template v-if="type === 'daily' && Array.isArray(content.data)">
        <div class="bg-slate-900/95 py-1.5">
        ...
        <div class="text-sm px-3 font-semibold bg-neutral-950/95 py-1">Partidas: {{ content.data.length }}</div>
        <div v-for="data in content.data" :key="data.champion_id + '-' + data.date" :class="data.is_remake ? 'bg-neutral-800/95' : data.result ? 'bg-green-900/95' : 'bg-red-900/95'">
      ...
      <template v-if="type === 'match' && !Array.isArray(content.data)">
        ...
        <div class="flex items-center justify-center px-3 py-1.5 gap-1.5" :class="content.data.is_remake ? 'bg-neutral-800/95' : content.data.result ? 'bg-green-900/95' : 'bg-red-900/95'">
